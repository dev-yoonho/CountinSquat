...
# Spring 서버 URL 설정
SPRING_SERVER_URL = "http://localhost:8080/api/receiveSquatCount"
...
def process_uploaded_video(file_path):
    ...
    while cap.isOpened():
        ...
        try:
            ...
            if squat_stage == 'up':
                ...
                if (hip_angle <= 100 or knee_angle <= 100):
                    squat_stage = 'down'
            elif squat_stage == 'down':
                if (hip_angle >= 120 or knee_angle >= 120):
    ...
    # 스쿼트 개수를 Spring 서버로 전송
    data = {
        "squatCount": uploaded_squat_count,
        "fileName": file_path
    }
    ...
    try:
        ...
        if response.status_code == 200:
            print("Successfully sent squat count to Spring server.")
        else:
            print(f"Failed to send squat count. Status code: {response.status_code}")
    except Exception as e:
        print(f"Error sending squat count to Spring server: {e}")
